...
router.post("/login", (req: Request, res: Response) => {
  ...
  // Foi alterado o secret exposto por uma função JWT
  const secret = process.env.JWT_SECRET;
  if (!secret) {
    return res
      .status(500)
      .json({ error: "Erro interno: JWT_SECRET não configurado." });
  }
...
router.get("/users/search", (req: Request, res: Response) => {
  ...
  const query =
    "SELECT id, username, email, balance FROM users WHERE username = ?"; /**troquei a concatenção dp username por um placeholder ? */

  db.all(query, [username], (err, rows) => {
    /** [username] como segundo argumento */
    db.close();
    if (err) {
      return res
        .status(500)
...
router.post("/transactions/transfer", (req: Request, res: Response) => {
  ...
  try {
    ...
    /** Alteração do catch vazio para um melhor tratamento */
    console.error("Falha na transferência:", error);
    return res
      .status(500)
      .json({ error: "Ocorreu um erro ao processar a transferência" });
  }
});
...
router.post("/reports/generate", (req: Request, res: Response) => {
  ...
  return res.status(200).json({
    ...
    output: `Relatorio estatico do tipo: ${type}`,
  });
});
